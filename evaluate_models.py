...
class ModelEvaluator:
    ...
    def get_evaluation_datasets(self, dataset_name, eval_datasets=None):
        """
        Get evaluation datasets with flexible control.

        Args:
            dataset_name: The dataset the model was trained on
            eval_datasets: Dict specifying evaluation datasets per training dataset
        """
        if eval_datasets is not None and dataset_name in eval_datasets:
            # Use custom evaluation datasets
            eval_list = []
            for eval_name in eval_datasets[dataset_name]:
                if eval_name == "Hopkins155":
                    eval_list.append(("Hopkins155", Hopkins155()))
                elif eval_name == "Hopkins12":
                    eval_list.append(("Hopkins12", Hopkins12()))
                elif eval_name == "KT3DMoSeg":
                    eval_list.append(("KT3DMoSeg", KT3DMoSeg()))
            return eval_list

        # Default behavior (cross-evaluation)
        eval_datasets_default = []

        if dataset_name == "hopk155":
            eval_datasets_default.append(("Hopkins155", Hopkins155()))
            eval_datasets_default.append(("Hopkins12", Hopkins12()))
        elif dataset_name == "hopk12":
            eval_datasets_default.append(("Hopkins12", Hopkins12()))
        elif dataset_name == "kt":
            eval_datasets_default.append(("KT3DMoSeg", KT3DMoSeg()))

        return eval_datasets_default
    ...
    def evaluate_single_model(
        self, model_path, include_orthogonality=True, eval_datasets=None
    ):
        """
        Evaluate a single model with flexible dataset control.

        Args:
            model_path: Path to the model file
            include_orthogonality: Whether to calculate orthogonality measures
            eval_datasets: Dict specifying which datasets to evaluate on
        """
        ...
        eval_dataset_list = self.get_evaluation_datasets(
            config["dataset"], eval_datasets
        )
        ...
        for algo in algorithms:
            error_rate = evaluate_model_performance(model, train_loader, algo, device)
        ...
        # Evaluate on specified external datasets
        for eval_name, eval_dataset in eval_dataset_list:
    ...
    def evaluate_models(
        self, model_names=None, include_orthogonality=True, eval_datasets=None
    ):
        """
        Evaluate models with flexible dataset control.

        Args:
            model_names: List of model names to evaluate (None for all)
            include_orthogonality: Whether to calculate orthogonality measures
            eval_datasets: Dict specifying which datasets to evaluate on, e.g.:
                          {'hopk155': ['Hopkins155', 'Hopkins12'],
                           'hopk12': ['Hopkins12', 'Hopkins155'],
                           'kt': ['KT3DMoSeg']}
                          If None, uses default cross-evaluation
        """
        ...
        for model_name in model_names:
            ...
            result = self.evaluate_single_model(
                str(model_path), include_orthogonality, eval_datasets
            )
    ...
    def create_comparison_table(self, df):
        ...
        additional_datasets = ["hopkins12", "hopkins155"]
        ...
        if "orthogonality_measure" in comparison_df.columns:
            comparison_df["orthogonality_measure"] = comparison_df[
    ...
    def create_heatmap_visualization(self, df, save_path="out/model_heatmap.png"):
        ...
        for col in error_cols:
            if heatmap_data[col].dtype == "object":
                heatmap_data[col] = (
                    heatmap_data[col].str.replace("%", "").str.replace("N/A", "nan")
                )
    ...
    def create_typst_table(
        self, df, save_path="out/model_table.typ", include_orthogonality=True
    ):
        display_df = df.copy()
        ...
        # Check for cross-evaluation data
        has_hopkins12 = False
        has_hopkins155 = False
        ...
        if "hopkins155_hierarchical_error" in df.columns:
            hopkins155_col = df["hopkins155_hierarchical_error"]
            has_hopkins155 = (hopkins155_col.notna() & (hopkins155_col != "N/A")).any()

        has_orthogonality = (
            include_orthogonality and "orthogonality_measure" in df.columns
        )

        # Calculate column count
        base_cols = 3  # Configuration, Train, Test
...
"""
        ...
        for _, row in display_df.iterrows():
            ...
            if has_hopkins155:
                hopkins155_error = row.get("hopkins155_hierarchical_error", "N/A")
                hopkins155_formatted = format_compact_error(hopkins155_error)
                row_line += f", [{hopkins155_formatted}]"
    ...
    table.cell(colspan: """
            + str(total_cols)
            + """)[#align(center)[evaluation on Hierarchical Clustering with Mean Clustering Error"""
        )
...
"""
    ...
    def create_tables(self, df, include_orthogonality=True, datasets_to_include=None):
        """
        Create Typst tables for specified datasets.

        Args:
            df: DataFrame with model results
            include_orthogonality: Whether to include orthogonality measures
            datasets_to_include: List of dataset names to create tables for.
                                Can use either short names ('hopk12', 'hopk155', 'kt')
                                or long names ('Hopkins12', 'Hopkins155', 'KT3DMoSeg')
        """
        unique_base_training_configs = df[
            ["dataset", "pretraining_epochs", "full_epochs"]
        ].drop_duplicates()

        # Normalize dataset names for comparison
        def normalize_dataset_name(name):
            ...
            return name_mapping.get(name.lower(), name.lower())

        if datasets_to_include is not None:
            datasets_to_include = [
                normalize_dataset_name(d) for d in datasets_to_include
            ]
        ...
        for _, config_row in unique_base_training_configs.iterrows():
            ...
            if datasets_to_include is not None and dataset not in datasets_to_include:
                continue
            ...
            if filtered_df.empty:
                print(
                    f"No data found for dataset {dataset} with {pre_epochs}/{full_epochs} epochs"
                )
                continue

            print(
                f"Creating table for dataset: {dataset} ({pre_epochs}/{full_epochs} epochs)"
            )
            ...
            self.create_heatmap_visualization(
                filtered_df, save_path.replace(".typ", "_heatmap.png")
            )
...
def main():
    ...
    # This will now work correctly for Hopkins12 dataset
    evaluator.create_tables(
        comparison_table, include_orthogonality=True, datasets_to_include=["hopk12"]
    )
...
if __name__ == "__main__":
    ...
    def evaluate_hopkins12_only():
        """Evaluate Hopkins12 models only on Hopkins12 dataset"""
        evaluator = ModelEvaluator()

        eval_config = {
            "hopk12": ["Hopkins12"]  # Only evaluate on same dataset
        }

        results_df = evaluator.evaluate_models(
            model_names=["hopk12_100_200_split_exfeat_ortho.pt"],  # Specific models
            include_orthogonality=True,
            eval_datasets=eval_config,
        )

        return results_df

    def evaluate_cross_dataset_only():
        """Evaluate models only on different datasets (cross-evaluation)"""
        evaluator = ModelEvaluator()

        cross_eval_config = {
            "hopk155": ["Hopkins12"],  # Hopkins155 models -> Hopkins12 data
            "hopk12": ["Hopkins155"],  # Hopkins12 models -> Hopkins155 data
            "kt": ["Hopkins155", "Hopkins12"],  # KT models -> both Hopkins datasets
        }

        results_df = evaluator.evaluate_models(
            include_orthogonality=True, eval_datasets=cross_eval_config
        )

        return results_df

    # Run specific evaluation strategies
    # evaluate_hopkins12_only()
    # evaluate_cross_dataset_only()
