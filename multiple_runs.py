...
def run_multiple_experiments(
    ...
    for run_idx in range(num_runs):
        ...
        main_train_loader, main_val_loader = get_train_val_loaders(
            ...
            strict_sequence_train_val_split=run_config["strict_sequence_train_val_split"],
            include_partial_sequences_train=run_config["include_partial_sequences_train"],
        ...
        print(f"Run {run_idx + 1} completed. Mean clustering error: {mean_clustering_error:.4f}")
    ...
    print(f"Average mean clustering error: {aggregated_metrics['mean_clustering_error_avg']:.4f}")
    print(f"Variance of mean clustering error: {aggregated_metrics['mean_clustering_error_var']:.6f}")
    print(f"Standard deviation of mean clustering error: {aggregated_metrics['mean_clustering_error_std']:.4f}")
    print(f"Minimum mean clustering error: {aggregated_metrics['mean_clustering_error_min']:.4f}")
    print(f"Maximum mean clustering error: {aggregated_metrics['mean_clustering_error_max']:.4f}")
    
    # Save results to file
    if save_results:
        timestamp = datetime.now().strftime("%Y%m%dT%H%M%S")
        results_file = os.path.join(results_dir, f"multiple_runs_results_{timestamp}{name}.json")
        
        # Convert numpy types to Python types for JSON serialization
        json_ready_metrics = copy.deepcopy(aggregated_metrics)
        json_ready_metrics["mean_clustering_error_avg"] = float(json_ready_metrics["mean_clustering_error_avg"])
        json_ready_metrics["mean_clustering_error_var"] = float(json_ready_metrics["mean_clustering_error_var"])
        json_ready_metrics["mean_clustering_error_std"] = float(json_ready_metrics["mean_clustering_error_std"])
        json_ready_metrics["mean_clustering_error_min"] = float(json_ready_metrics["mean_clustering_error_min"])
        json_ready_metrics["mean_clustering_error_max"] = float(json_ready_metrics["mean_clustering_error_max"])
...
def load_and_analyze_results(results_file: str) -> Dict[str, Any]:
    ...
    print(f"Variance of mean clustering error: {results['mean_clustering_error_var']:.6f}")
    print(f"Standard deviation of mean clustering error: {results['mean_clustering_error_std']:.4f}")
