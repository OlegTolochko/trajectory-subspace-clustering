...
def train_model(config):
    ...
    optimizer_stage1 = optim.Adam(model.feature_extractor.parameters(), lr=config["learning_rate"], weight_decay=config["weight_decay"])
    optimizer_stage2 = optim.Adam(model.parameters(), lr=config["learning_rate"], weight_decay=config["weight_decay"])
    ...
    pretrained_model = pretraining_loop(config=config, model=model, train_loader=train_loader, device=device, optimizer=optimizer_stage1, scheduler=scheduler_stage1, metrics=metrics_to_log)
    fully_trained_model = full_training_loop(config=config, model=pretrained_model, train_loader=train_loader, device=device, optimizer=optimizer_stage2, scheduler=scheduler_stage2, metrics=metrics_to_log)
    
    return fully_trained_model


def pretraining_loop(config, model, train_loader, device, optimizer, scheduler, metrics):
    for epoch in tqdm(range(config["pretraining_epochs"]), desc="Pretraining Model"):
        ...
        metrics["infonce_loss"] = epoch_loss_stage1 / num_seq_processed if num_seq_processed > 0 else 0.0
        
        wandb.log(metrics, step=epoch+1)
        
        print(f"Pretraining Epoch {epoch + 1}/{config["pretraining_epochs"]}, Avg Loss: {metrics["infonce_loss"]:.4f}")
    
    return model


def full_training_loop(config, model, train_loader, device, optimizer, scheduler, metrics):
    for epoch in tqdm(range(config["full_epochs"]), desc="Training Full Model", miniters=10):
        model.train()
        ...
        for batch_data in train_loader:
            ...
            if config["use_sequence_randomization"]:
                seq_x_train = randomize_sequences_for_class(seq_x=seq_x, seq_labels=seq_labels, epoch=epoch, device=device)
            else:
                seq_x_train = seq_x
        ...
        print(f"Full Training Epoch {epoch + 1}/{config["full_epochs"]}, Avg Loss: {metrics["total_loss"]:.4f}")
        print(f"Epoch {epoch + 1}/{config["full_epochs"]}: InfoNCE Loss: {metrics["infonce_loss"]:.4f}, Res Loss: {metrics["residual_loss"]:.4f}, Feat Loss: {metrics["feat_diff_loss"]:.4f}, Ortho Loss: {metrics["ortho_loss"]:.4f}")
